...
- [Contract Wizard](https://wizard.openzeppelin.com/)
  ...
  // Featured Research — CTO-level depth
  {
    slug: 'bridge-hack-analysis',
    title: 'Bridge Hack Analysis: Patterns and Prevention',
    category: 'research',
    difficulty: 'Hard',
    lastUpdated: '2025-11-15',
    content: `
# Bridge Hack Analysis: Patterns and Prevention

Bridge exploits have dominated DeFi losses. We analyze attack patterns from Ronin, Wormhole, Nomad, and others to extract actionable security principles.

## Attack Surface Overview

Cross-chain bridges must handle: message relay, signature verification, liquidity management, and consensus assumptions. Each layer introduces trust assumptions that attackers exploit.

## Case: Replay and Validation Bypass

**Nomad Bridge (Aug 2022)** — $190M. The root cause: a failed initialization left \`committedRoot\` as zero. Attackers could replay any message by providing a valid Merkle proof against the zero root. Lesson: **Initialization and invariant checks are critical.** A single unset storage slot broke the entire security model.

\`\`\`solidity
// Vulnerable pattern: Zero root accepted
function process(bytes memory _message) public {
    bytes32 leaf = keccak256(_message);
    require(merkleProof.verify(committedRoot, leaf), "Invalid proof");
    // committedRoot was 0 — any proof against 0 worked
    _executeMessage(_message);
}
\`\`\`

## Case: Signature Verification Flaws

**Wormhole (Feb 2022)** — $320M. A signature verification bypass: the verifier did not validate that all required guardians had signed. One compromised key sufficed. Lesson: **Enforce quorum and key diversity.** Verify \`signatures.length >= threshold\` and reject duplicate signers.

## Case: Private Key Compromise

**Ronin Bridge (Mar 2022)** — $625M. Five of nine validator keys compromised via social engineering. Lesson: **Key distribution and operational security matter as much as code.** Consider MPC, threshold schemes, and geographic distribution.

## Prevention Checklist for Bridge Architects

1. **Message uniqueness**: Nonce + chainId + sender in signed payload. Reject replays.
2. **Quorum**: Require N-of-M signatures from distinct validators. No single-point-of-failure.
3. **Delay**: Implement timelock for large withdrawals. Gives time to pause on anomaly.
4. **Invariants**: Formalize and test (e.g., totalLocked == sum(per-chain)). Use Foundry invariants.
5. **Initialization**: Explicit init validation. Reject zero/invalid roots and configs.
6. **Upgrades**: Timelock + governance. No single-admin emergency upgrades without delay.

## Recommended Reading

- [Nomad Post-Mortem](https://medium.com/nomad-xyz-blog)
- [Wormhole Incident Report](https://wormholecrypto.medium.com)
- [Bridge Security Framework (L2BEAT)](https://l2beat.com)
    `.trim(),
  },
  {
    slug: 'l2-security-model',
    title: 'L2 Security Model: Sequencer, Provers, and Data Availability',
    category: 'research',
    difficulty: 'Hard',
    lastUpdated: '2025-11-12',
    content: `
# L2 Security Model: Sequencer, Provers, and Data Availability

Layer 2 architectures (Optimistic Rollups, ZK Rollups) introduce new trust assumptions. Understanding them is essential for auditors and protocol integrators.

## The L2 Trust Stack

| Layer | Optimistic (OP, Arbitrum) | ZK Rollup (zkSync, StarkNet) |
|-------|---------------------------|------------------------------|
| Execution | Sequencer (centralized) | Sequencer |
| Validity | Fraud proof (7d challenge) | Validity proof (instant) |
| Data | Batches to L1 | Batches + proofs to L1 |
| Upgrade | Timelock + multisig | Timelock + multisig |

## Sequencer Risk

The sequencer orders transactions. It can:
- Censor transactions (temporary)
- Reorder for MEV (economic)
- Fail or go offline (availability)

**Mitigation**: Force-include mechanism (L1 → L2 priority queue), decentralized sequencer sets (future), and escape hatches for withdrawals without sequencer.

## Data Availability (DA)

If batched data is not published to L1, a malicious sequencer could hide state. Withdrawals and fraud proofs depend on data being available. **Blob DA** (EIP-4844) reduces cost while preserving availability guarantees. Auditors should verify: withdrawal logic assumes data availability within a known window.

## Fraud Proof vs Validity Proof

**Optimistic**: Assume valid until proven otherwise. Challenge period (e.g., 7 days) allows anyone to submit fraud proof. Contracts must support challenge logic and bond slashing.

**ZK**: Proof of validity submitted with each batch. No challenge period. Trust shifts to the prover and circuit correctness. Audit the verifier contract and circuit assumptions.

## Upgrade Risk

L2 upgradeability is typically controlled by a multisig + timelock. Key questions:
- Who can upgrade? (Often "security council" + governance)
- How long is the timelock? (48h–7d common)
- Can upgrades change core logic? (Beware of "upgrade to steal" scenarios)

## Audit Priorities for L2 Protocols

1. **Cross-messaging**: L1↔L2 message passing, finality assumptions, replay protection
2. **Withdrawal delays**: Withdrawal finality time, escape hatch usability
3. **Sequencer assumptions**: What breaks if sequencer is malicious or offline?
4. **Upgrade paths**: Who can upgrade, timelock, and scope of upgradeable logic
5. **Bridged asset handling**: Canonical bridge vs third-party, mint/burn symmetry

## References

- [Ethereum L2 Roadmap](https://ethereum.org/en/layer-2/)
- [L2BEAT Risk Framework](https://l2beat.com/scaling/risk)
- [Optimism Bedrock Docs](https://docs.optimism.io)
    `.trim(),
  },
  {
    slug: 'reentrancy-edge-cases',
    title: 'Reentrancy Edge Cases: Beyond the Classic Pattern',
    category: 'research',
    difficulty: 'Hard',
    lastUpdated: '2025-11-10',
    content: `
# Reentrancy Edge Cases: Beyond the Classic Pattern

Classic reentrancy (withdraw before balance update) is well-known. Real-world exploits often involve cross-function, cross-contract, and read-only reentrancy. This article covers patterns that slip past basic checks.

## Cross-Function Reentrancy

Two functions share state. Attacker re-enters through a *different* function.
...
function transfer(address to, uint amount) external {
    require(!locked[msg.sender]);  // Same lock — good
    require(balances[msg.sender] >= amount);
    balances[msg.sender] -= amount;
    balances[to] += amount;
}
...
If \`transfer\` didn't check \`locked\`, attacker could: withdraw → receive callback → call \`transfer\` (to move tokens from another internal accounting) before withdraw completes. **Use a single ReentrancyGuard for all state-changing external-call functions**, or ensure *all* functions that touch shared state respect the same lock.

## Read-Only Reentrancy

The vulnerable contract makes no external *transfer* in the callback, but an external viewer (oracle, integrator) *reads* state during the callback. State is inconsistent mid-transaction.

\`\`\`solidity
// Victim: DEX pair
function swap(...) external {
    // ... update reserves after swap ...
    IUniswapV2Callee(msg.sender).uniswapV2Call(...);  // Callback
    _update(balance0, balance1, reserve0, reserve1);   // State update AFTER
}
\`\`\`

Attacker's callback: calls an *oracle* that reads \`getReserves()\`. Reserves are stale; oracle returns wrong price. Attacker profits on another contract that trusts the oracle. **Fix**: Update all state (reserves, etc.) *before* the callback. CEI (Checks-Effects-Interactions) applies to *any* external call, not just transfers.

## ERC-777 and ERC-721 Callbacks

\`tokensReceived\`, \`onERC721Received\` — these hooks are designed to re-enter. If your contract receives such tokens and does logic in the same call flow, ensure no inconsistent state is exposed. Treat token callbacks like \`receive()\` and \`fallback()\`.

## Delegation and Proxy Patterns

A proxy delegates to an implementation. If the implementation has reentrancy, the proxy's storage can be read/written mid-call. Upgradeable contracts are especially tricky: implementation changes, but storage layout must remain consistent. **Audit the full call graph** when implementation makes external calls.

## Practical Checklist

1. **CEI everywhere**: Effects (state updates) before Interactions (external calls).
2. **Single ReentrancyGuard** covering all functions that (a) change shared state and (b) make external calls.
3. **Read-only reentrancy**: Consider whether external observers read your state during callbacks. Update state before callbacks.
4. **Token hooks**: ERC-777, ERC-721 — assume reentrancy. Guard or restructure.
5. **Proxies**: Trace delegation; implementation reentrancy affects proxy storage.
6. **Fuzz with Foundry**: Invariant "totalSupply == sum(balances)" can catch balance inconsistencies from reentrancy.

## References

- [SWC-107: Reentrancy](https://swcregistry.io/docs/SWC-107)
- [Consensys Reentrancy](https://consensys.github.io/smart-contract-best-practices/attacks/reentrancy/)
- [Rekt News: Read-only Reentrancy](https://rekt.news)
    `.trim(),
  },
]
